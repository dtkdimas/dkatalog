<x-app-layout>
    ...
    <div class="card p-5">
        <h3 class="h3 fw-bold mb-48">{{ __('Form Edit Catalogue') }}</h3>
        <div class="">
                ...
                <div class="space-y-6">
                    <div class="mb-3">
                        ...
                        <input type="text" name="catalogue_name"
                            value="{{ old('catalogue_name', $catalogue->catalogue_name) }}" class="form-control"
                            id="catalogue_name" required>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="brand_id" id="brand_id" class="form-select" required
                            {{ request('brand_id') ? 'disabled' : '' }}>
                            <option value="" selected disabled>{{ __('Select Brand') }}</option>
                            @foreach ($brands as $brand)
                                <option value="{{ $brand->id }}"
                                    {{ old('brand_id', $catalogue->brand_id) == $brand->id ? 'selected' : '' }}>
                                    {{ $brand->brand_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="url" name="catalogue_banner" class="form-control"
                            value="{{ old('catalogue_banner', $catalogue->catalogue_banner) }}" id="catalogue_banner">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="url" name="catalogue_url"
                            value="{{ old('catalogue_url', $catalogue->catalogue_url) }}" class="form-control"
                            id="catalogue_url" required>
